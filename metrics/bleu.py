...
class BLEUWithBonus(BLEU):
    ...
    def add_inst(self,
        ...
        BLEU.add_inst(self, candi, ref_list)
        if yn_label is not None and yn_ref is not None:
            self.add_yn_bonus(candi, ref_list, yn_label, yn_ref)
        elif entity_ref is not None:
            self.add_entity_bonus(candi, entity_ref)

    def add_yn_bonus(self, candi, ref_list, yn_label, yn_ref):
        for n_size in range(self.n_size):
            candi_ngram = common.get_ngram(candi, n_size, label=yn_label)
            ref_ngram = []
            for ref_id, r in enumerate(yn_ref):
                ref_ngram.append(common.get_ngram(ref_list[ref_id], n_size, label=r))
            match_size, candi_size = common.get_match_size(candi_ngram, ref_ngram)
            self.match_ngram[n_size] += self.alpha * match_size
            self.candi_ngram[n_size] += self.alpha * match_size

    def add_entity_bonus(self, candi, entity_ref):
        for n_size in range(self.n_size):
            candi_ngram = common.get_ngram(candi, n_size, label='ENTITY')
            ref_ngram = []
